...
for (let i = 0; i < allSessionKeys.length; i++) {
  ...
  console.log(
    'Key',
    i,
    `: prefix: ${prefix}, addr: ${addr}, sessionId: ${sessionId}`,
  );
  ...
  let vestingSessionBytes = vestingSessions[0]
    ? vestingSessions[0]
    : new Uint8Array();

  let vestingSession = new Args(vestingSessionBytes).nextSerializable(
    VestingSessionInfo,
  );

  // console.log("  - ", vestingSession);
  assert(
    vestingSession.toAddr === addr,
    'Vesting session info addr and key are !=',
  );
  console.log(
    '  totalAmount:',
    vestingSession.totalAmount.toString(),
    'MAS',
    `(${vestingSession.totalAmount} nanoMAS)`,
  );
  console.log(
    '  initialReleaseAmount:',
    vestingSession.initialReleaseAmount.toString(),
    'MAS',
    `(${vestingSession.initialReleaseAmount} nanoMAS)`,
  );
  let start = new Date(Number(vestingSession.startTimestamp)).toUTCString();
  console.log('  startTimestamp (UTC)', start);

  let claimedAmountKey = new Args()
    .addU8(3)
    .addString(addr)
    .addU64(sessionId)
    .serialize();
  ...
  let claimedAmountBytes = claimedAmounts[0]
    ? claimedAmounts[0]
    : new Uint8Array();
